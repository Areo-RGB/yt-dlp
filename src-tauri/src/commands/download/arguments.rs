...
pub(super) fn build_download_args(
    ...
    // Remove emoji before expanding the title-based directory and filename.
    args.push("--replace-in-metadata".to_string());
    args.push("title".to_string());
    args.push(EMOJI_PATTERN.to_string());
    args.push(String::new());

    // 输出路径模板
    let output_template =
        build_output_template(&params.download_dir, params.output_template.as_deref());
...
#[cfg(test)]
mod output_template_tests {
    use super::{build_output_template, DEFAULT_OUTPUT_TEMPLATE};
    use std::path::PathBuf;

    #[test]
    fn puts_downloads_in_a_title_directory() {
        let expected = PathBuf::from("/downloads")
            .join("%(title).200s")
            .join("%(title)s.%(ext)s")
            .to_string_lossy()
            .to_string();
        assert_eq!(
            build_output_template("/downloads", Some("%(title)s.%(ext)s")),
            expected
        );
    }

    #[test]
    fn uses_the_default_filename_inside_the_title_directory() {
        let expected = PathBuf::from("/downloads")
            .join("%(title).200s")
            .join(DEFAULT_OUTPUT_TEMPLATE)
            .to_string_lossy()
            .to_string();
        assert_eq!(build_output_template("/downloads", None), expected);
    }
}
